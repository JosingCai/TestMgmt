...
def get_cabitnet_id():
    ...
    if not resp.ok:
        ...
        response = resp.json()
        server_cabinet_id = int(response["content"]["id"])
    return {"server_cabinet_id": server_cabinet_id, "server_room_id": server_room_id, "idc_id": idc_id}


def device_sql(number):
    REGION_IDs = [1, 2]
    handle = Database()
    MFGs = ["Dell", "HP", "Huawei", "Lenovo", "H3C", "Inspur", "Sugon", "Gooxi", "Greatwall", "Supermicro", "KVM", "VMware"]
    MODELs = ["PowerEdge R620", "2488 V5", "8100 V3", "RH8100 V5", "PowerEdge R940"]
    ...
    cabinet_sum = number // 14 + 1
    for num in range(cabinet_sum):
        print("cabinet no: ", num)
        cabinet_dict = get_cabitnet_id()
        begin_unit = 1
        for device_num in range(14):
            SN = ''.join(random.sample(string.ascii_letters + string.digits, 20))
            asset_number = SN
            remark = ''.join(random.sample(string.ascii_letters + string.digits, 30))
            updated_at = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            created_at = updated_at
            bootos_active_time = updated_at
            idc_id = cabinet_dict['idc_id']
            server_room_id = cabinet_dict['server_room_id']
            server_cabinet_id = cabinet_dict['server_cabinet_id']
            ip_raw = random.sample(range(1,256), 4)
            ip_str= map(str,ip_raw)
            ip_oob = '.'.join(ip_str)
            ip_raw = random.sample(range(1,256), 4)
            ip_str= map(str,ip_raw)
            ip_biz = '.'.join(ip_str)
            ...
            chassis_type = random.choice(CHASSIS_TYPEs)
            level = random.choice(LEVELs)
            region_id = random.choice(REGION_IDs)
            source = random.choice(SOURCEs)
            p_status = random.choice(P_STATUSs)
            d_status = random.choice(D_STATUSs)
            print(created_at, updated_at,region_id, SN, mfg, model, arch, chassis_type, asset_number, SN, ip_biz, level,  bootos_active_time, ip_oob, idc_id, server_room_id, server_cabinet_id, begin_unit, p_status, d_status, source, remark)
            cmd_device = "INSERT INTO device SET created_at = '%s', updated_at = '%s', deleted_at = null, tenant = 'default', region_id = %d, sn = '%s', manufacturer = '%s', model = '%s', arch = '%s', chassis_type = '%s', spec = '12C32G1113G', cpu = '{\"items\": [{\"type\": \"Central Processor\", \"cores\": 6, \"flags\": [\"FPU (Floating-point unit on-chip)\", \"VME (Virtual mode extension)\", \"DE (Debugging extension)\", \"PSE (Page size extension)\", \"TSC (Time stamp counter)\", \"MSR (Model specific registers)\", \"PAE (Physical address extension)\", \"MCE (Machine check exception)\", \"CX8 (CMPXCHG8 instruction supported)\", \"APIC (On-chip APIC hardware supported)\", \"SEP (Fast system call)\", \"MTRR (Memory type range registers)\", \"PGE (Page global enable)\", \"MCA (Machine check architecture)\", \"CMOV (Conditional move instruction supported)\", \"PAT (Page attribute table)\", \"PSE-36 (36-bit page size extension)\", \"CLFSH (CLFLUSH instruction supported)\", \"DS (Debug store)\", \"ACPI (ACPI supported)\", \"MMX (MMX technology supported)\", \"FXSR (FXSAVE and FXSTOR instructions supported)\", \"SSE (Streaming SIMD extensions)\", \"SSE2 (Streaming SIMD extensions 2)\", \"SS (Self-snoop)\", \"HTT (Multi-threading)\", \"TM (Thermal monitor supported)\", \"PBE (Pending break enabled)\"], \"model\": \"Intel(R) Xeon(R) CPU E5-2420 @ 1.90GHz\", \"family\": \"Xeon\", \"threads\": 12, \"voltage\": \"0.8 V\", \"l1_cache\": \"0x001B\", \"l2_cache\": \"0x001C\", \"l3_cache\": \"0x001D\", \"max_speed\": \"1900 MHz\", \"manufacturer\": \"Intel(R) Corporation\", \"current_speed\": \"1900 MHz\", \"enabled_cores\": 6, \"socket_designation\": \"CPU01\"}, {\"type\": \"Central Processor\", \"cores\": 6, \"flags\": [\"FPU (Floating-point unit on-chip)\", \"VME (Virtual mode extension)\", \"DE (Debugging extension)\", \"PSE (Page size extension)\", \"TSC (Time stamp counter)\", \"MSR (Model specific registers)\", \"PAE (Physical address extension)\", \"MCE (Machine check exception)\", \"CX8 (CMPXCHG8 instruction supported)\", \"APIC (On-chip APIC hardware supported)\", \"SEP (Fast system call)\", \"MTRR (Memory type range registers)\", \"PGE (Page global enable)\", \"MCA (Machine check architecture)\", \"CMOV (Conditional move instruction supported)\", \"PAT (Page attribute table)\", \"PSE-36 (36-bit page size extension)\", \"CLFSH (CLFLUSH instruction supported)\", \"DS (Debug store)\", \"ACPI (ACPI supported)\", \"MMX (MMX technology supported)\", \"FXSR (FXSAVE and FXSTOR instructions supported)\", \"SSE (Streaming SIMD extensions)\", \"SSE2 (Streaming SIMD extensions 2)\", \"SS (Self-snoop)\", \"HTT (Multi-threading)\", \"TM (Thermal monitor supported)\", \"PBE (Pending break enabled)\"], \"model\": \"Intel(R) Xeon(R) CPU E5-2420 @ 1.90GHz\", \"family\": \"Xeon\", \"threads\": 12, \"voltage\": \"0.8 V\", \"l1_cache\": \"0x0020\", \"l2_cache\": \"0x0021\", \"l3_cache\": \"0x0022\", \"max_speed\": \"1900 MHz\", \"manufacturer\": \"Intel(R) Corporation\", \"current_speed\": \"1900 MHz\", \"enabled_cores\": 6, \"socket_designation\": \"CPU02\"}], \"total_cores\": 12, \"total_threads\": 24, \"total_physicals\": 2}', memory = '{\"items\": [{\"size\": 8589934592, \"type\": \"DDR3\", \"speed\": \"1333 MHz\", \"location\": \"DIMM010\", \"asset_tag\": \"Unknown\", \"part_number\": \"M393B1K70CH0-YH9\", \"manufacturer\": \"Samsung\", \"serial_number\": \"0x33427585\", \"configured_voltage\": \"\"}, {\"size\": 0, \"type\": \"DDR3\", \"speed\": \"Unknown\", \"location\": \"DIMM011\", \"asset_tag\": \"NO DIMM\", \"part_number\": \"NO DIMM\", \"manufacturer\": \"NO DIMM\", \"serial_number\": \"NO DIMM\", \"configured_voltage\": \"\"}, {\"size\": 8589934592, \"type\": \"DDR3\", \"speed\": \"1333 MHz\", \"location\": \"DIMM020\", \"asset_tag\": \"Unknown\", \"part_number\": \"M393B1K70CH0-YH9\", \"manufacturer\": \"Samsung\", \"serial_number\": \"0x33427376\", \"configured_voltage\": \"\"}, {\"size\": 0, \"type\": \"DDR3\", \"speed\": \"Unknown\", \"location\": \"DIMM021\", \"asset_tag\": \"NO DIMM\", \"part_number\": \"NO DIMM\", \"manufacturer\": \"NO DIMM\", \"serial_number\": \"NO DIMM\", \"configured_voltage\": \"\"}, {\"size\": 0, \"type\": \"DDR3\", \"speed\": \"Unknown\", \"location\": \"DIMM030\", \"asset_tag\": \"NO DIMM\", \"part_number\": \"NO DIMM\", \"manufacturer\": \"NO DIMM\", \"serial_number\": \"NO DIMM\", \"configured_voltage\": \"\"}, {\"size\": 0, \"type\": \"DDR3\", \"speed\": \"Unknown\", \"location\": \"DIMM031\", \"asset_tag\": \"NO DIMM\", \"part_number\": \"NO DIMM\", \"manufacturer\": \"NO DIMM\", \"serial_number\": \"NO DIMM\", \"configured_voltage\": \"\"}, {\"size\": 8589934592, \"type\": \"DDR3\", \"speed\": \"1333 MHz\", \"location\": \"DIMM110\", \"asset_tag\": \"Unknown\", \"part_number\": \"M393B1K70CH0-YH9\", \"manufacturer\": \"Samsung\", \"serial_number\": \"0x33427381\", \"configured_voltage\": \"\"}, {\"size\": 0, \"type\": \"DDR3\", \"speed\": \"Unknown\", \"location\": \"DIMM111\", \"asset_tag\": \"NO DIMM\", \"part_number\": \"NO DIMM\", \"manufacturer\": \"NO DIMM\", \"serial_number\": \"NO DIMM\", \"configured_voltage\": \"\"}, {\"size\": 8589934592, \"type\": \"DDR3\", \"speed\": \"1333 MHz\", \"location\": \"DIMM120\", \"asset_tag\": \"Unknown\", \"part_number\": \"M393B1K70CH0-YH9\", \"manufacturer\": \"Samsung\", \"serial_number\": \"0x85A85F03\", \"configured_voltage\": \"\"}, {\"size\": 0, \"type\": \"DDR3\", \"speed\": \"Unknown\", \"location\": \"DIMM121\", \"asset_tag\": \"NO DIMM\", \"part_number\": \"NO DIMM\", \"manufacturer\": \"NO DIMM\", \"serial_number\": \"NO DIMM\", \"configured_voltage\": \"\"}, {\"size\": 0, \"type\": \"DDR3\", \"speed\": \"Unknown\", \"location\": \"DIMM130\", \"asset_tag\": \"NO DIMM\", \"part_number\": \"NO DIMM\", \"manufacturer\": \"NO DIMM\", \"serial_number\": \"NO DIMM\", \"configured_voltage\": \"\"}, {\"size\": 0, \"type\": \"DDR3\", \"speed\": \"Unknown\", \"location\": \"DIMM131\", \"asset_tag\": \"NO DIMM\", \"part_number\": \"NO DIMM\", \"manufacturer\": \"NO DIMM\", \"serial_number\": \"NO DIMM\", \"configured_voltage\": \"\"}], \"total_size\": 34359738368, \"maximum_size\": 824633720832, \"number_of_devices\": 12, \"number_of_used_devices\": 4}', nic = '{\"items\": [{\"ip\": \"192.168.2.100\", \"mac\": \"04:f9:38:93:3d:3f\", \"link\": \"yes\", \"port\": 0, \"side\": \"inside\", \"type\": \"Ethernet\", \"model\": \"Broadcom Inc. and subsidiaries NetXtreme BCM5719 Gigabit Ethernet PCIe (rev 01)\", \"speed\": \"1Gbit/s\", \"location\": \"eth0\", \"pci_slot\": 0, \"switch_ref\": null, \"bus_address\": \"0000:02:00.0\", \"manufacturer\": \"Broadcom\", \"firmware_version\": \"5719-v1.31 NCSI v1.2.12.0\"}, {\"ip\": \"\", \"mac\": \"04:f9:38:93:3d:40\", \"link\": \"no\", \"port\": 1, \"side\": \"inside\", \"type\": \"Ethernet\", \"model\": \"Broadcom Inc. and subsidiaries NetXtreme BCM5719 Gigabit Ethernet PCIe (rev 01)\", \"speed\": \"1Gbit/s\", \"location\": \"eth1\", \"pci_slot\": 0, \"switch_ref\": null, \"bus_address\": \"0000:02:00.1\", \"manufacturer\": \"Broadcom\", \"firmware_version\": \"5719-v1.31 NCSI v1.2.12.0\"}, {\"ip\": \"\", \"mac\": \"04:f9:38:93:3d:41\", \"link\": \"no\", \"port\": 2, \"side\": \"inside\", \"type\": \"Ethernet\", \"model\": \"Broadcom Inc. and subsidiaries NetXtreme BCM5719 Gigabit Ethernet PCIe (rev 01)\", \"speed\": \"1Gbit/s\", \"location\": \"eth2\", \"pci_slot\": 0, \"switch_ref\": null, \"bus_address\": \"0000:02:00.2\", \"manufacturer\": \"Broadcom\", \"firmware_version\": \"5719-v1.31 NCSI v1.2.12.0\"}, {\"ip\": \"\", \"mac\": \"04:f9:38:93:3d:42\", \"link\": \"no\", \"port\": 3, \"side\": \"inside\", \"type\": \"Ethernet\", \"model\": \"Broadcom Inc. and subsidiaries NetXtreme BCM5719 Gigabit Ethernet PCIe (rev 01)\", \"speed\": \"1Gbit/s\", \"location\": \"eth3\", \"pci_slot\": 0, \"switch_ref\": null, \"bus_address\": \"0000:02:00.3\", \"manufacturer\": \"Broadcom\", \"firmware_version\": \"5719-v1.31 NCSI v1.2.12.0\"}]}', logical_disk = '{\"items\": [{\"name\": \"/dev/sda\", \"size\": \"1113.9 GB, 1195997396992 bytes\"}, {\"name\": \"/dev/sda1\", \"size\": \"0.0 GB, 1048576 bytes\"}, {\"name\": \"/dev/sda2\", \"size\": \"0.2 GB, 268435456 bytes\"}, {\"name\": \"/dev/sda3\", \"size\": \"2.0 GB, 2147483648 bytes\"}, {\"name\": \"/dev/sda4\", \"size\": \"0.0 GB, 1024 bytes\"}, {\"name\": \"/dev/sda5\", \"size\": \"1111.6 GB, 1193578332160 bytes\"}], \"total_size\": 2391992697856}', physical_disk = '{\"items\": [{\"wwn\": \"\", \"size\": 298998443278, \"slot\": \"0\", \"model\": \"HUS156030VLS600 \", \"bus_type\": \"SAS\", \"location\": \"13:0\", \"media_type\": \"HDD\", \"error_count\": 0, \"part_number\": \"\", \"inquiry_data\": \"\", \"manufacturer\": \"\", \"foreign_state\": \"\", \"serial_number\": \"\", \"firmware_state\": \"\", \"transfer_speed\": \"\", \"firmware_version\": \"\"}, {\"wwn\": \"\", \"size\": 298998443278, \"slot\": \"1\", \"model\": \"HUS156030VLS600 \", \"bus_type\": \"SAS\", \"location\": \"13:1\", \"media_type\": \"HDD\", \"error_count\": 0, \"part_number\": \"\", \"inquiry_data\": \"\", \"manufacturer\": \"\", \"foreign_state\": \"\", \"serial_number\": \"\", \"firmware_state\": \"\", \"transfer_speed\": \"\", \"firmware_version\": \"\"}, {\"wwn\": \"\", \"size\": 298998443278, \"slot\": \"2\", \"model\": \"HUS156030VLS600 \", \"bus_type\": \"SAS\", \"location\": \"13:2\", \"media_type\": \"HDD\", \"error_count\": 0, \"part_number\": \"\", \"inquiry_data\": \"\", \"manufacturer\": \"\", \"foreign_state\": \"\", \"serial_number\": \"\", \"firmware_state\": \"\", \"transfer_speed\": \"\", \"firmware_version\": \"\"}, {\"wwn\": \"\", \"size\": 298998443278, \"slot\": \"3\", \"model\": \"HUS156030VLS600 \", \"bus_type\": \"SAS\", \"location\": \"13:3\", \"media_type\": \"HDD\", \"error_count\": 0, \"part_number\": \"\", \"inquiry_data\": \"\", \"manufacturer\": \"\", \"foreign_state\": \"\", \"serial_number\": \"\", \"firmware_state\": \"\", \"transfer_speed\": \"\", \"firmware_version\": \"\"}], \"total_size\": 1195993773112}', raid = '{\"items\": [{\"id\": \"0\", \"mode\": \"\", \"model\": \"SAS2208\", \"pci_address\": \"00:01:00:00\", \"serial_number\": \"\", \"firmware_version\": \"3.190.05-1669\"}]}', oob = '{\"user\": [{\"id\": 2, \"name\": \"yunji\", \"privilege_level\": 4}, {\"id\": 3, \"name\": \"cloudboot1\", \"privilege_level\": 0}, {\"id\": 4, \"name\": \"user\", \"privilege_level\": 2}, {\"id\": 5, \"name\": \"operator\", \"privilege_level\": 3}, {\"id\": 6, \"name\": \"callback\", \"privilege_level\": 2}, {\"id\": 7, \"name\": \"OEMUser\", \"privilege_level\": 0}, {\"id\": 8, \"name\": \"voidint\", \"privilege_level\": 3}, {\"id\": 9, \"name\": \"admin\", \"privilege_level\": 4}, {\"id\": 10, \"name\": \"test\", \"privilege_level\": 4}, {\"id\": 11, \"name\": \"keji\", \"privilege_level\": 0}, {\"id\": 12, \"name\": \"readonly\", \"privilege_level\": 4}], \"network\": {\"ip\": \"192.168.1.164\", \"mac\": \"78:d7:52:93:78:23\", \"ip_src\": \"dhcp\", \"gateway\": \"192.168.1.1\", \"netmask\": \"255.255.0.0\"}, \"firmware\": \"7.31\"}', bios = '{\"manufacturer\": \"Insyde Corp.\", \"release_date\": \"02/11/2018\", \"characteristics\": [\"PCI is supported\", \"BIOS is upgradeable\", \"BIOS shadowing is allowed\", \"Boot from CD is supported\", \"Selectable boot is supported\", \"EDD is supported\", \"Japanese floppy for NEC 9800 1.2 MB is supported (int 13h)\", \"Japanese floppy for Toshiba 1.2 MB is supported (int 13h)\", \"5.25/360 kB floppy services are supported (int 13h)\", \"5.25/1.2 MB floppy services are supported (int 13h)\", \"3.5/720 kB floppy services are supported (int 13h)\", \"3.5/2.88 MB floppy services are supported (int 13h)\", \"8042 keyboard services are supported (int 9h)\", \"CGA/mono video services are supported (int 10h)\", \"ACPI is supported\", \"USB legacy is supported\", \"BIOS boot specification is supported\", \"Targeted content distribution is supported\", \"UEFI is supported\"], \"firmware_version\": \"RMIBV518\"}', fan = '{\"items\": [{\"speed\": \"2400 RPM\", \"location\": \"FAN1 F Speed\"}, {\"speed\": \"2160 RPM\", \"location\": \"FAN1 R Speed\"}, {\"speed\": \"2400 RPM\", \"location\": \"FAN2 F Speed\"}, {\"speed\": \"2040 RPM\", \"location\": \"FAN2 R Speed\"}, {\"speed\": \"2520 RPM\", \"location\": \"FAN3 F Speed\"}, {\"speed\": \"2040 RPM\", \"location\": \"FAN3 R Speed\"}, {\"speed\": \"2520 RPM\", \"location\": \"FAN4 F Speed\"}, {\"speed\": \"2160 RPM\", \"location\": \"FAN4 R Speed\"}]}', power_supply = '{\"items\": []}', board = '{\"manufacturer\": \"Huawei Technologies Co., Ltd.\", \"product_name\": \"BC11SRSF1\", \"serial_number\": \"021SKT4ME3000465\", \"onboard_devices\": [], \"firmware_version\": \"V100R002\"}', pci = '{\"items\": [{\"id\": 1, \"type\": \"x4 PCI Express x4\", \"length\": \"Other\", \"pci_device\": null, \"bus_address\": \"0000:00:01.1\", \"designation\": \"PCIE1\", \"current_usage\": \"Available\"}, {\"id\": 2, \"type\": \"x4 PCI Express x4\", \"length\": \"Other\", \"pci_device\": null, \"bus_address\": \"0000:00:1c.4\", \"designation\": \"PCIE2\", \"current_usage\": \"Available\"}, {\"id\": 4, \"type\": \"x16 PCI Express x16\", \"length\": \"Other\", \"pci_device\": null, \"bus_address\": \"0000:80:03.0\", \"designation\": \"PCIE4\", \"current_usage\": \"Available\"}, {\"id\": 5, \"type\": \"x8 PCI Express x8\", \"length\": \"Other\", \"pci_device\": null, \"bus_address\": \"0000:80:01.0\", \"designation\": \"PCIE5\", \"current_usage\": \"Available\"}], \"total_slots\": 4}', hba = null, lldp = '{\"lldp\": {\"interface\": {\"eth0\": {\"age\": \"0 day, 00:00:16\", \"rid\": \"1\", \"via\": \"LLDP\", \"port\": {\"id\": {\"type\": \"ifname\", \"value\": \"GigabitEthernet1/0/13\"}, \"mfs\": \"10240\", \"descr\": \"GigabitEthernet1/0/13 Interface\", \"power\": {\"class\": \"class 0\", \"pairs\": \"signal\", \"source\": \"PSE\", \"enabled\": false, \"priority\": \"critical\", \"allocated\": \"0\", \"requested\": \"0\", \"supported\": false, \"power-type\": \"2\", \"device-type\": \"PSE\", \"paircontrol\": false}, \"auto-negotiation\": {\"current\": \"1000BaseTFD - Four-pair Category 5 UTP, full duplex mode\", \"enabled\": true, \"supported\": true}}, \"vlan\": {\"pvid\": true, \"value\": \"VLAN 0002\", \"vlan-id\": \"2\"}, \"ppvid\": {\"enabled\": false, \"supported\": true}, \"chassis\": {\"iDCOS-H3C-Switch1\": {\"id\": {\"type\": \"mac\", \"value\": \"8c:e6:87:63:01:43\"}, \"ttl\": \"120\", \"descr\": \"H3C Switch S5120-52P-SI Software Version 5.20, Release 1515 Copyright(c) 2004-2015 Hangzhou H3C Tech. Co., Ltd. All rights reserved.\", \"mgmt-ip\": \"192.168.2.254\", \"capability\": [{\"type\": \"Bridge\", \"enabled\": true}, {\"type\": \"Router\", \"enabled\": true}]}}}}}}', extra = null, asset_number = '%s', biz_ip = '[\"%s\"]', os = 'ubuntu14.04', hostname = 'ubuntu-test', buyer = '张三', buy_time = '2020-07-23', level = '%d', application = '测试', `usage` = '测试测得都对', label = '[\"test\", \"dev\", \"ops\"]', bootos_active_time = '%s', oob_ip = '%s', oob_user = 'admin', oob_password = '7JhjFABmhkzwThULoRCnnPoOuGEtZ4ReK3T2038=', idc_id = %d, server_room_id = %d, server_cabinet_id = %d, begin_unit = %d, height = 2, locator = 5, blade_enclosure_sn = 'blade_sn', hmc_id = 0, power_status = '%s', status = '%s', source = '%s', creator = '6126694f-038f-4049-a964-3ad445e4d47d', remark = '%s'" %(created_at, updated_at,region_id, SN, mfg, model, arch, chassis_type, asset_number, ip_biz, level,  bootos_active_time, ip_oob, idc_id, server_room_id, server_cabinet_id, begin_unit, p_status, d_status, source, remark)
            # print("cmd_devie: ", cmd_device)
            handle.insert_sql(cmd_device)
            begin_unit = begin_unit + 3
...
def device_location_sql(number):
    ...
    STATUSs = ['repaired', 'ignored', 'uncheck', 'checked']
    SOURCE_NAMEs = ["OOB", "Inspection", "WEB", "IPMI"]
    MFGs = ["Dell", "HP", "Huawei", "Lenovo", "H3C", "Inspur", "Sugon", "Gooxi", "Greatwall", "Supermicro", "KVM", "VMware"]
    MODELs = ["PowerEdge R620", "2488 V5", "8100 V3", "RH8100 V5", "PowerEdge R940"]
    ARCHs = ["aarch64", "x86_64" ,"ppc64"]
    CHASSIS_TYPEs = ["unknown", "rack_server", "blade_server", "minicomputer"]
    ...
    for ID in range(number):
        ...
        severity = random.choice(SEVERITYs)
        ...
        cmd_215 = "INSERT INTO event_warning (created_at, updated_at, deleted_at, sn, category, severity, source_id, source_name, is_checked, creator, tenant, remark, message, source_time) VALUES ('2020-05-21 11:38:18', '2020-05-21 11:38:18', null, '%s', '%s', '%s', %d, 'Inspection', '0', '', 'default', '', '%s', '%s');"%(SN, category, severity, source_id, message,source_time)
        cmd_380 = "INSERT INTO event_warning (created_at, updated_at, deleted_at, sn, category, severity, source_id, source_name, status, creator, tenant, remark, message, source_time) VALUES ('2020-07-22 10:41:57', '2020-07-22 10:41:57', null, '%s', '%s', '%s', %d, '%s','%s', '', 'default', '', '%s', '%s');" %(SN, category, severity, source_id, source_name, status, message,source_time)
        print("No.", ID)
        handle.insert_sql(cmd_3X0)
    handle.close_handler()
